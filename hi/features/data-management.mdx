---
title: "डेटा प्रबंधन"
description: "एकीकृत डेटा प्रबंधन इंटरफ़ेस, फॉर्म, प्रक्रिया, कार्य, उप-फॉर्म आदि डेटा संचालन का समर्थन करता है।"
icon: "database"
---

OpenYida एकीकृत डेटा प्रबंधन CLI प्रदान करता है, फॉर्म डेटा, प्रक्रिया इंस्टेंस, कार्य, उप-फॉर्म आदि के लिए पूर्ण CRUD संचालन का समर्थन करता है।

---

## अवलोकन

<CardGroup cols={2}>
  <Card title="फॉर्म डेटा" icon="table">
    फॉर्म इंस्टेंस डेटा क्वेरी, बनाएं, अपडेट करें, हटाएं।
  </Card>
  <Card title="प्रक्रिया डेटा" icon="workflow">
    प्रक्रिया इंस्टेंस, अनुमोदन रिकॉर्ड, प्रक्रिया स्थिति प्रबंधित करें।
  </Card>
  <Card title="कार्य प्रबंधन" icon="check-circle">
    लंबित, पूर्ण, मेरे द्वारा सबमिट किए गए, मुझे CC किए गए कार्यों की क्वेरी करें।
  </Card>
  <Card title="उप-फॉर्म" icon="list">
    उप-फॉर्म डेटा को स्वतंत्र रूप से क्वेरी और प्रबंधित करें।
  </Card>
</CardGroup>

---

## कमांड प्रारूप

```bash
openyida data <action> <resource> [args]
```

- **action**: `query` | `get` | `create` | `update` | `execute`
- **resource**: `form` | `process` | `task` | `subform` | `operation-records`

---

## फॉर्म डेटा प्रबंधन

### फॉर्म डेटा क्वेरी

```bash
# बुनियादी क्वेरी
openyida data query form <appType> <formUuid>

# पेजिनेशन क्वेरी
openyida data query form <appType> <formUuid> --page 1 --size 50

# सशर्त क्वेरी
openyida data query form <appType> <formUuid> \
  --search-json '{"field_xxx": "value"}'

# विशिष्ट इंस्टेंस क्वेरी
openyida data query form <appType> <formUuid> --inst-id <formInstId>
```

### एकल रिकॉर्ड प्राप्त करें

```bash
openyida data get form <appType> --inst-id <formInstId>
```

### डेटा बनाएं

```bash
openyida data create form <appType> <formUuid> \
  --data '{"field_xxx": "value", "field_yyy": 123}' \
  --dept-id <departmentId>
```

आप `--data` पैरामीटर में फ़ील्ड ID के बजाय फ़ील्ड लेबल का भी उपयोग कर सकते हैं। CLI स्वचालित रूप से फ़ील्ड लेबल का पता लगाता है और फ़ॉर्म स्कीमा प्राप्त करके उन्हें संबंधित फ़ील्ड ID में परिवर्तित करता है।

```bash
# फ़ील्ड ID के बजाय फ़ील्ड लेबल का उपयोग
openyida data create form <appType> <formUuid> \
  --data '{"नाम": "राहुल", "आयु": 30, "विभाग": "इंजीनियरिंग"}'
```

<Info>
  फ़ील्ड लेबल रिज़ॉल्यूशन लेबल टेक्स्ट को फ़ॉर्म स्कीमा से मिलान करके काम करता है। एक ही `--data` ऑब्जेक्ट में फ़ील्ड लेबल और फ़ील्ड ID दोनों स्वीकार किए जाते हैं — जो कुंजी किसी ज्ञात लेबल से मेल नहीं खाती, वह यथावत पास की जाती है।
</Info>

### डेटा अपडेट करें

```bash
openyida data update form <appType> \
  --inst-id <formInstId> \
  --data '{"field_xxx": "new value"}' \
  --use-latest-version y
```

---

## प्रक्रिया डेटा प्रबंधन

### प्रक्रिया इंस्टेंस क्वेरी

```bash
# प्रक्रिया सूची क्वेरी
openyida data query process <appType> <formUuid> \
  --page 1 --size 20 \
  --instance-status RUNNING

# उपलब्ध स्थिति: RUNNING (चल रहा है), COMPLETED (पूर्ण), TERMINATED (समाप्त)
```

### प्रक्रिया विवरण प्राप्त करें

```bash
openyida data get process <appType> --process-inst-id <processInstanceId>
```

### प्रक्रिया इंस्टेंस बनाएं

```bash
openyida data create process <appType> <formUuid> \
  --process-code <processCode> \
  --data '{"field_xxx": "value"}' \
  --dept-id <departmentId>
```

### अनुमोदन रिकॉर्ड क्वेरी

```bash
openyida data query operation-records <appType> \
  --process-inst-id <processInstanceId>
```

---

## कार्य प्रबंधन

### कार्य सूची क्वेरी

```bash
# लंबित कार्य
openyida data query tasks <appType> --type todo

# पूर्ण कार्य
openyida data query tasks <appType> --type done

# मेरे द्वारा सबमिट किए गए
openyida data query tasks <appType> --type submitted

# मुझे CC किए गए
openyida data query tasks <appType> --type cc
```

### कार्य निष्पादित करें

```bash
openyida data execute task <appType> \
  --task-id <taskId> \
  --process-inst-id <processInstanceId> \
  --out-result AGREE \
  --remark "सहमत, कृपया जारी रखें" \
  --data '{"comment": "अनुमोदन पारित"}'
```

**out-result विकल्प:**
- `AGREE` - सहमत
- `DISAGREE` - असहमत

---

## उप-फॉर्म प्रबंधन

### उप-फॉर्म डेटा क्वेरी

```bash
openyida data query subform <appType> <formUuid> \
  --inst-id <formInstId> \
  --table-field-id <fieldId> \
  --page 1 --size 20
```

---

## उन्नत उपयोग

### जटिल क्वेरी स्थितियां

```bash
# बहु-फ़ील्ड सशर्त क्वेरी
openyida data query form <appType> <formUuid> \
  --search-json '{
    "field_status": "approved",
    "field_amount": { "$gte": 1000 },
    "field_created_at": { "$gte": "2024-01-01" }
  }'
```

### बैच ऑपरेशन स्क्रिप्ट उदाहरण

```bash
#!/bin/bash
# फॉर्म डेटा बैच निर्यात

APP_TYPE="APP_XXXXXXXX"
FORM_UUID="FORM-XXXX-XXXX"
PAGE=1

while true; do
  RESULT=$(openyida data query form $APP_TYPE $FORM_UUID --page $PAGE --size 100)
  
  # वर्तमान पृष्ठ डेटा प्रोसेस करें
  echo "$RESULT" >> ./export.json
  
  # जांचें कि क्या अगला पृष्ठ है
  HAS_MORE=$(echo "$RESULT" | jq '.hasMore')
  if [ "$HAS_MORE" != "true" ]; then
    break
  fi
  
  PAGE=$((PAGE + 1))
done
```

---

## डेटा निर्यात उदाहरण

```bash
# निर्दिष्ट समय अवधि का डेटा निर्यात करें
openyida data query form <appType> <formUuid> \
  --search-json '{
    "field_created_at": {
      "$gte": "2024-01-01T00:00:00",
      "$lte": "2024-12-31T23:59:59"
    }
  }' \
  --size 100 > ./data-export.json
```

---

## ध्यान दें

<Warning>
  डेटा संचालन स्थायी हैं, हटाने और अपडेट संचालन पूर्ववत नहीं किए जा सकते। बैच संचालन निष्पादित करने से पहले छोटे पैमाने पर परीक्षण करने की अनुशंसा की जाती है।
</Warning>

<Info>
  पेजिनेशन क्वेरी डिफ़ॉल्ट रूप से प्रति पृष्ठ 20 रिकॉर्ड, अधिकतम 100 का समर्थन करती है। बड़ी मात्रा में डेटा निर्यात के लिए पेजिनेशन लूप का उपयोग करने की अनुशंसा की जाती है।
</Info>
