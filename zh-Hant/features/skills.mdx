...
openyida login --check-only        # 僅檢查登入狀態，不觸發登入
...
# 初始化工作目錄
openyida copy [--force]            # 複製 project 目錄（--force 覆蓋已有檔案）
openyida copy -skills              # 複製 yida-skills 到 AI 工具 skills 目錄
openyida copy -project [--force]   # 顯式複製 project 目錄（與預設行為相同）

# 診斷工具
openyida doctor [--fix]            # 環境診斷（--repair 是 --fix 的別名）
openyida doctor --production --app <appId>  # 線上應用診斷
openyida doctor --monitor          # 健康監控
openyida doctor --report <format>  # 生成報告 (json|markdown|html)
openyida doctor --create-ticket    # 根據診斷結果建立工單
openyida doctor --create-voc       # 建立 VOC（需求回饋）
openyida doctor --auto-submit      # 自動判斷並提交工單或 VOC
```
...
```bash
# 創建應用
openyida create-app "<名稱>" [desc] [icon] [iconColor] [colour] [navTheme] [layoutDirection]
