...
查詢應用程式中所有表單和頁面項目，結果以 JSON 格式輸出到標準輸出。
...
| 參數 | 說明 |
|------|------|
| `<appType>` | 應用程式 ID（如 `APP_XXXXX`） |

| 選項 | 說明 |
|------|------|
| `--keyword <text>` | 按關鍵字篩選結果（匹配表單名稱、UUID、類型或路徑） |
...
<Info>
  執行此命令前必須先登入。如果尚未認證，請先執行 `openyida login`。
</Info>
...
偵測和修復宜搭專屬資料庫中 PostgreSQL Sequence 起始值問題。當資料遷移或同步導致 Sequence 當前值小於表中最大主鍵 ID 時，新插入資料會因主鍵衝突而失敗。`db-seq-fix` 命令可自動識別並修正這些不一致。
...
| 參數 | 說明 |
|------|------|
| *（無參數）* | 檢查所有 Sequence 狀態（預設） |
| `--fix` | 自動修復不同步的 Sequence |
| `--dry-run` | 預覽修復 SQL 語句，不實際執行 |
| `--help` | 顯示幫助信息 |

### 監控的表

該命令監控以下核心表的 Sequence：
...
<Warning>
  在正式環境執行 `--fix` 前，請先執行 `--dry-run` 預覽變更。在高流量期間執行修復可能會導致短暫的 ID 分配衝突。
</Warning>

<Info>
  執行此命令前需要先登入並擁有專屬資料庫存取權限。如尚未認證，請先執行 `openyida login`。
</Info>
