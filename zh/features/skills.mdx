...
查询应用中所有表单和页面条目，结果以 JSON 格式输出到标准输出。
...
| 参数 | 说明 |
|------|------|
| `<appType>` | 应用 ID（如 `APP_XXXXX`） |

| 选项 | 说明 |
|------|------|
| `--keyword <text>` | 按关键词过滤结果（匹配表单名称、UUID、类型或路径） |
...
<Info>
  运行此命令前必须先登录。如果尚未认证，请先运行 `openyida login`。
</Info>
...
检测和修复宜搭专属数据库中 PostgreSQL Sequence 起始值问题。当数据迁移或同步导致 Sequence 当前值小于表中最大主键 ID 时，新插入数据会因主键冲突而失败。`db-seq-fix` 命令可自动识别并修正这些不一致。
...
| `--fix` | 自动修复不同步的 Sequence |
| `--dry-run` | 预览修复 SQL 语句，不实际执行 |
| `--help` | 显示帮助信息 |

### 监控的表

该命令监控以下核心表的 Sequence：
...
<Warning>
  在生产环境执行 `--fix` 前，请先运行 `--dry-run` 预览变更。在高流量期间执行修复可能会导致短暂的 ID 分配冲突。
</Warning>

<Info>
  运行此命令前需要先登录并拥有专属数据库访问权限。如尚未认证，请先运行 `openyida login`。
</Info>
